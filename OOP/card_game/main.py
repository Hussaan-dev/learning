SUITS: list[str] = ["Clubs", "Diamonds", "Hearts", "Spades"]

RANKS: list[str] = [
    "2","3","4","5","6","7","8","9","10",
    "Jack","Queen","King","Ace",
]


class Card:
    def __init__(self, rank: str, suit: str) -> None:
        self.rank=rank
        self.suit=suit
        self.rank_index=RANKS.index(rank)
        self.suit_index=SUITS.index(suit)

    def __eq__(self, other: "Card") -> bool:
        return (self.rank_index == other.rank_index 
                and self.suit_index == other.suit_index)

    def __lt__(self, other: "Card") -> bool:
        if self.rank_index == other.rank_index:
            return self.suit_index < other.suit_index
        return self.rank_index < other.rank_index

    def __gt__(self, other: "Card") -> bool:
        return  other < self

    def __str__(self) -> str:
        return f"{self.rank} of {self.suit}"


class Round:
    def __init__(self,card1: "Card",card2: "Card"):
        self.card1=card1
        self.card2=card2
        
    def resolve_round(self) -> int:
        raise NotImplementedError

class HighCardRound(Round):
    def resolve_round(self) -> int:
        if self.card1 > self.card2:
            return 1
        if self.card1 < self.card2:
            return 2
        return 0

class LowCardRound(Round):
    def resolve_round(self) -> int:
        if self.card1 < self.card2:
            return 1
        if self.card1 > self.card2:
            return 2
        return 0

def main() -> None:
    card_1 = Card("Ace", "Spades")
    card_2 = Card("King", "Hearts")
    card_3 = Card("Ace", "Clubs")

    print("Cards:")
    print(card_1)
    print(card_2)
    print(card_3)

    print("\nCard Comparisons:")
    print(f"Is {card_1} higher than {card_2}? {card_1 > card_2}")
    print(f"Is {card_3} lower than {card_1}? {card_3 < card_1}")

    print("\nHigh Card Round:")
    high_round = HighCardRound(card_1, card_2)
    winner = high_round.resolve_round()

    if winner == 0:
        print("Result: Tie")
    else:
        print(f"Winner: Player {winner}")

    print("\nLow Card Round:")
    low_round = LowCardRound(card_1, card_2)
    winner = low_round.resolve_round()

    if winner == 0:
        print("Result: Tie")
    else:
        print(f"Winner: Player {winner}")
    
main()